(function() {
  ...
  PriorityQueue = function(options) {
    ...
    var self = {
      ...
      /**
       * SPECIFIC TO A NODERECORD
       */
      // Find a NodeRecord that has the specified Node
      find: function(node) {
        for(var i = contents.length - 1; i >= 0; i--) {
          if(contents[i].object.node === node) {
            return contents[i].object;
          }
        }
      ...
      // Remove a NodeRecord from the queue
      remove: function(object) {
        if(!sorted) {
          sort();
        }

        element = contents.splice( this.getIndex(object), 1 );
      ...
      // Check to see if the queue contains a NodeRecord that has the specified Node
